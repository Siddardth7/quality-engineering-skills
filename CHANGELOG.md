...
### Added
- FastMCP tools for Production Part Approval Process (PPAP) in `quality_mcp.tools.ppap` (`audit_ppap_package`, `lookup_ppap_requirement`, `validate_psw`, `assess_ppap_capability`, `render_ppap_canvas`) exposing AIAG PPAP 4th Edition 18-element package completeness auditing, Table 4.1 matrix lookup, 27-field PSW validation, Section 2.2.11 initial capability analysis, and visual canvas rendering with strict Section 5 Customer Authority Invariant enforcement (#107).
