...
### Added
- Deterministic Nonconformance Reporting (NCR) defect-statement and disposition recommendation engine in `quality_core.ncr.nonconformance` (`write_nonconformance`, `recommend_disposition`, `NonconformanceWriteResult`, `DispositionRecommendation`) enforcing ISO 9001:2015 §8.7 and IATF 16949:2016 §8.7 objective-evidence phrasing, blame phrase filtration, root-cause speculation detection with RCA guidance, disposition routing across Scrap, Rework, UseAsIs, ReturnToVendor, and Regrade with cited rationale and approval authority, and a mandatory load-bearing `INSUFFICIENT_DATA` negative control on ambiguous defect data (#93).
