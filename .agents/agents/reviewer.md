...
3. Assess:
   ...
   - **Scope & Out-of-Scope Diff Audit**: Strictly check for scope creep. Is ANY file touched in the diff that was not explicitly in the spec's target file list or directly required for the issue? Do NOT rationalize out-of-scope symptom fixes (e.g. editing unrelated tests or dead code) — flag them as MUST-FIX findings and require reverting or logging separate issues.
   - Does it honor the Definition of Done (#43) and the coverage gates?
   - Are any scratch files or unwanted files in the diff (`spikes/`, `marketing/`, etc.)?
...
Be the last line of defense. Green tests are not the same as correct behavior — if the code is wrong or touches out-of-scope files, say BLOCK or NEEDS WORK.
