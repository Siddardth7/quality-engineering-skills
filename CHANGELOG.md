...
- **`/8d` AI agent skill — `skills/8d-problem-solving/SKILL.md`** (E12, Milestone 11, #215). The
  ...
  **The skill asserts no standard of its own**: it instructs the agent to read and relay the
  ...
  five-mutation negative control and a non-vacuity guard. No `quality-core` / `quality-mcp` source
  change, no new `CITATIONS.tsv` row and no new Process Design Decision — a skill asserts no
  standard.
