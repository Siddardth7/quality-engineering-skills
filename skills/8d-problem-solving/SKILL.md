...
- *Strict Invariant — closure precondition:* When the current `state` is `D8`, the adjacent target is `CLOSED`, and **closing is the one transition that asserts the whole report is finished.** Call `advance_8d(..., target="CLOSED")` **only if** the most recent `validate_8d` result carries `closeable: true`. If it carries `closeable: false`, do not attempt the transition at all: report every entry in `gate_reasons` verbatim — `code`, `message`, and `rule_id` — and stop. `advance_8d` evaluates only the gates belonging to the transition attempted, so a whole-report reason that belongs to an earlier step — `LINKED_NCR_INVALID` (`PDD-8D-008`) gates D3 → D4, not D8 → CLOSED — does **not** block it. Attempting the transition anyway returns `ADVANCED` and yields a `CLOSED` report that `validate_8d` still reports as not closeable. Refusing to *report* that report as closeable is not enough; the transition itself must not be attempted.
...
- `advance_8d` evaluates only the gates belonging to the transition attempted. It is not a second opinion on `validate_8d`'s whole-report `closeable` flag: an `ADVANCED` verdict never licenses reporting a report as closeable while `gate_reasons` is non-empty, and — per the closure precondition above — never licenses *performing* the D8 → CLOSED transition while `closeable` is `false`.
