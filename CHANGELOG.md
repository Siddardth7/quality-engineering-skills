...
### Added
- Deterministic Cost of Poor Quality (COPQ) financial estimator engine in `quality_core.copq.estimator` (`estimate_copq`, `COPQEstimationResult`) implementing standard Prevention-Appraisal-Failure (PAF) cost accounting, itemized internal failure calculations (scrap, rework labor and added material, containment sorting, retest, downtime), itemized external failure calculations (warranty claims, customer returns, product recalls, price concessions), aggregate COPQ, CoGQ conformance costs, total Cost of Quality (CoQ), %-of-revenue, and failure cost distribution ratios cited against the ASQ Certified Six Sigma Green Belt (CSSGB) Body of Knowledge and CSSC Lean Six Sigma Manual with mandatory negative controls (#94).
