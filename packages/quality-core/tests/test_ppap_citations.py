"""Citation integrity for the PPAP docs — every AIAG quotation is real and located (Issue #103).
...
def _blockquote_blocks(markdown: str) -> list[tuple[int, str]]:
    ...
    for number, raw in enumerate(markdown.splitlines(), start=1):
        ...
        if fenced:
            continue
        if raw.startswith(">"):
            text = raw.lstrip("> ").rstrip().strip('"').strip("'")
            if text:
...
def test_negative_control_tsv_delimiter_detection() -> None:
    ...
    comma_content = "site,src_line,quote\nRULE 1,313,Some quote\n"
    reader = csv.reader(comma_content.splitlines(), delimiter="\t")
    rows = list(reader)
    assert len(rows[0]) == 1, (
        "Comma-separated content parsed with tab delimiter must yield 1 column"
    )
